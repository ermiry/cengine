...
CENGINE_PUBLIC void ui_font_delete (void *font_ptr);;

CENGINE_PUBLIC FontSource *font_source_get_by_size (Font *font, unsigned int size);
...
CENGINE_EXPORT Font *ui_font_create (const char *font_name, const char *font_filename);

// sets the font sizes to be loaded
// returns 0 on success, 1 on error
CENGINE_EXPORT u8 ui_font_set_sizes (Font *font, u8 n_sizes, ...);

// loads the font with the specified values
// returns 0 on success, 1 on error
CENGINE_EXPORT u8 ui_font_load (Font *font, struct _Renderer *renderer, int style);

// gets a refrence to the default font --> the first one that was added
CENGINE_EXPORT Font *ui_font_get_default (void);

// gets a refrence to a ui font by its name
CENGINE_EXPORT Font *ui_font_get_by_name (const char *name);

/*** Misc ***/

CENGINE_PRIVATE u32 get_code_point_from_UTF8 (const char **c, u8 advancePtr);

/*** Main ***/

// called by internal cengine methods
// inits cengine ui font capabilities
CENGINE_PRIVATE u8 ui_fonts_init (void);

// called by internal cengine methods
// destroys any cengine font data left
CENGINE_PRIVATE void ui_font_end (void);
