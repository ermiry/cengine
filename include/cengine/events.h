...
CENGINE_PUBLIC EventAction *cengine_event_register (CengineEventType type, Action action, void *args);

// unregister from an event
// returns 0 on succes, 1 on error
CENGINE_PUBLIC u8 cengine_event_unregister (EventAction *event_action);

// called by internal cengine methods to trigger all the actions
// that have been registered to an event
CENGINE_PUBLIC void cengine_event_trigger (CengineEventType type, void *event_data);

/*** main ***/

CENGINE_PRIVATE u8 cengine_events_init (void);

CENGINE_PRIVATE u8 cengine_events_end (void);
