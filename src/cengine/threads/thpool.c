/**********************************
 * This is thread pool an implementation based on the thpool created by: Johan Hanssen Seferidis
 *
 **********************************/
...
static struct thpool_ *thpool_new (void) {

	struct thpool_ *thpool = (struct thpool_ *) malloc (sizeof (struct thpool_));
	if (thpool) {
		memset (thpool, 0, sizeof (struct thpool_));
		thpool->threads = NULL;
	}
...
struct thpool_ *thpool_init (int num_threads) {
	...
	struct thpool_ *thpool = thpool_new ();
	if (thpool) {
		...
		#ifdef CENGINE_DEBUG
		cengine_log_msg (stderr, ERROR, NO_TYPE, "Failed to allocate a new thpool!");
		#endif
	}
